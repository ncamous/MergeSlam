...
bool MergeSystem::optimizationIteration(int itsPerTry, float minChange)
{
	...
	std::cout<<"Accseing to Lock 1"<<std::endl;
...
void MergeSystem::startLiveMerge()
 ...
 // graph_sub = nh.subscribe("/lsd1/lsd_slam/graph", 10, &MergeSystem::graphCb, this); //lsd1 while reconstructing graph throws exception of sophus::scalenotpositive rxso3.hpp
  
	keyFrames_sub2 = nh.subscribe("/lsd2/lsd_slam/keyframes", 200, &MergeSystem::keyFrameCb, this);
	graph_sub2 = nh.subscribe("/lsd2/lsd_slam/graph",15, &MergeSystem::graphCb, this);
...
void MergeSystem::keyFrameCb(lsd_slam_viewer::keyframeMsgConstPtr msg)
{
 ...
 // printSim3(poseKeyFrame);
  ...
  //printSim3(currentKeyFrame->pose->getCamToWorld());
    ...
    // Update KeyFrame Graph Variables
  //keyFrameGraph.at(camId)->allFramePoses.push_back(currentKeyFrame.at(camId)->pose);
	  ...
	  for(unsigned int i = 0; i < msg->numFrames; i++)
	  {
		...
		Eigen::aligned_allocator< std::pair<const int, std::shared_ptr<Frame> > > >::iterator notYet= keyFrameGraph.at(msg->camId)->idToKeyFrame.end();
			if(itFrame!=notYet)
			{
				...
				Frame* frameGraph=refGraphFrame.get(); 
//				keyFrameGraph.at(msg->camId)->keyframesAll.push_back(keyFrameGraph.at(msg->camId)->idToKeyFrame.find(framePoseData[i].id)->second);
			...
			//	printSim3(poseKeyFrame);
				memcpy(poseKeyFrame.data(),camToWorld.data(), sizeof(double)*7);		
				frameGraph->pose->setPoseExternal(poseKeyFrame);
				keyFrameGraph.at(msg->camId)->keyframesAllMutex.lock_shared();
				keyFrameGraph.at(msg->camId)->keyframesAll.push_back(frameGraph);
				keyFrameGraph.at(msg->camId)->keyframesAllMutex.unlock_shared();
...
*/
	...
	if(time_counter > deltaT)
	{
		...
		std::cout<<"num frames "<<msg->numFrames<<std::endl;
        for(unsigned int i=0;i<msg->numFrames;i++)
		{
						std::unordered_map< int, std::shared_ptr<Frame>, std::hash<int>, std::equal_to<int>,
		Eigen::aligned_allocator< std::pair<const int, std::shared_ptr<Frame> > > >::iterator itFrame= keyFrameGraph.at(msg->camId)->idToKeyFrame.find(framePoseData[i].id);
		...
		Eigen::aligned_allocator< std::pair<const int, std::shared_ptr<Frame> > > >::iterator notYet= keyFrameGraph.at(msg->camId)->idToKeyFrame.end();
				if(itFrame!=notYet)
				{
					...
					newConstraintMutex.lock();	
					keyFrameGraph.at(msg->camId)->addKeyFrame(frameGraph);
					newConstraintMutex.unlock();
				}
		}
...
void MergeSystem::matchCb(coslam_msgs::keyframeMatchInfoConstPtr msg)
{
  ...
  //std::cout<<"Initial Sim3 Transformation"<< std::endl;
  //printSim3(BtoA_initial);
  
		Constraint.push_back(new KFConstraintStruct());
		
//		if(int id= currentKeyFrame.at(msg->iCam)->id()== msg->kfId1)
	//	{	
			ROS_INFO("ok");
			std::shared_ptr<Frame> reffirst = keyFrameGraph.at(cam_from)->idToKeyFrame.find(msg->kfId1)->second;
			std::shared_ptr<Frame> refsecond = keyFrameGraph.at(cam_to)->idToKeyFrame.find(msg->kfId2)->second;
	//		parentKeyFrame = currentKeyFrame.at(camID)->hasTrackingParent() ? currentKeyFrame.at(camID)->getTrackingParent() : 0;
			Constraint.back()->firstFrame = reffirst.get();
			Constraint.back()->secondFrame =refsecond.get();//parentKeyFrame;
			Constraint.back()->secondToFirst = Constraint.back()->firstFrame->getScaledCamToWorld().inverse() * Constraint.back()->secondFrame->getScaledCamToWorld();
			Constraint.back()->information  <<
					0.8098,-0.1507,-0.0557, 0.1211, 0.7657, 0.0120, 0,
					-0.1507, 2.1724,-0.1103,-1.9279,-0.1182, 0.1943, 0,
			...
			keyFrameGraph.at(msg->iCam)->insertConstraint(Constraint.back());
