...
void KeyFrameGraphDisplay::draw(float* color)
{
	dataMutex.lock();
	numRefreshedAlready = 0;
	...
	{
		if(showKFCameras)
			keyframes[i]->drawCam(2*lineTesselation, color);
	...
	*/
	...
	if(showConstraints)
	{
		...
		for(unsigned int i=0;i<constraints.size();i++)
		{
			...
			//double colorScalar = std::max(0.0, std::min(1.0, constraints[i].err / 0.05));
			//glColor3f(colorScalar, 1 - colorScalar, 0);
			glColor3f(0.6*color[0], 0.6*color[1],0.6* color[2] );
...
void KeyFrameGraphDisplay::drawNoPC(float* color)
{
	dataMutex.lock();
	numRefreshedAlready = 0;
...
void KeyFrameGraphDisplay::addGraphMsg(lsd_slam_viewer::keyframeGraphMsgConstPtr msg)
{
	...
	assert(msg->constraintsData.size() == sizeof(GraphConstraint)*msg->numConstraints); //aborts if expression to compare equal to zero
