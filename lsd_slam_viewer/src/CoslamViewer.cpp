...
CoslamViewer::~CoslamViewer()
{
	...
	delete currentCamDisplay2;
	delete graphDisplay;
	delete graphDisplay2;
}


void CoslamViewer::reset()
{
	...
	currentCamDisplay2 = new KeyFrameDisplay();
	graphDisplay2 = new KeyFrameGraphDisplay();
...
void CoslamViewer::addFrameMsg(lsd_slam_viewer::keyframeMsgConstPtr msg)
{
	meddleMutex.lock();
...
void CoslamViewer::addFrameMsg2(lsd_slam_viewer::keyframeMsgConstPtr msg)
{
	meddleMutex.lock();

	if(!msg->isKeyframe)
	{
		if(currentCamDisplay2->id > msg->id)
		{
			printf("detected backward-jump in id (%d to %d), resetting!\n", currentCamDisplay->id, msg->id);
			resetRequested = true;
		}
		currentCamDisplay2->setFrom(msg);
		lastAnimTime2 = lastCamTime2 = msg->time;
		lastCamID2 = msg->id;
	}
	else
		graphDisplay2->addMsg(msg);

	meddleMutex.unlock();
}


void CoslamViewer::addGraphMsg(lsd_slam_viewer::keyframeGraphMsgConstPtr msg)
{
	meddleMutex.lock();
...
void CoslamViewer::addGraphMsg2(lsd_slam_viewer::keyframeGraphMsgConstPtr msg)
{
	meddleMutex.lock();

	graphDisplay2->addGraphMsg(msg);

	meddleMutex.unlock();
}


void CoslamViewer::init()
{
	setAnimationPeriod(30);
	startAnimation();
}

QString CoslamViewer::helpString() const
{
	return QString("");
}

void CoslamViewer::draw()
{
	meddleMutex.lock();
	...
	// Colors for camera (Keyframes)
	float colorC1[3];
	colorC1[0] =  1.0f; colorC1[1] =  0.0f; colorC1[2] =  0.0f;
		
	float colorC2[3];
	colorC2[0] =  0.0f; colorC2[1] =  0.0f; colorC2[2] =  1.0f;
		
	// Color for current camera 
	float color[3];
	color[0] =  0.0f; color[1] =  1.0f; color[2] =  0.0f;
	
	if(showCurrentCamera){
	  
		currentCamDisplay->drawCam(2*lineTesselation, color);
		currentCamDisplay2->drawCam(2*lineTesselation, color);
	}    
	if(showCurrentPointcloud){
		currentCamDisplay->drawPC(pointTesselation, 1);
		currentCamDisplay2->drawPC(pointTesselation, 1);	      
	} 

	graphDisplay->draw(colorC1);
	//graphDisplay->draw(colorC1);
	graphDisplay2->drawNoPC(colorC2);
...
void CoslamViewer::keyReleaseEvent(QKeyEvent *e)
  {

  }


void CoslamViewer::setToVideoSize()
{
	this->setFixedSize(1600,900);
}


void CoslamViewer::remakeAnimation()
{
	delete kfInt;
	kfInt = new qglviewer::KeyFrameInterpolator(new qglviewer::Frame());
...
void CoslamViewer::keyPressEvent(QKeyEvent *e)
  {
    switch (e->key())
    {
      case Qt::Key_S :
