...
void frameCb2(lsd_slam_viewer::keyframeMsgConstPtr msg)
{

	if(msg->time > lastFrameTime) return;

	if(viewer != 0)
		viewer->addFrameMsg2(msg);
}
...
void graphCb2(lsd_slam_viewer::keyframeGraphMsgConstPtr msg)
{
	if(viewer != 0)
		viewer->addGraphMsg2(msg);
}
...
void rosThreadLoop( int argc, char** argv )
{
	...
	*/
	...
	//For camera 1
	ros::Subscriber liveFrames_sub = nh.subscribe("/lsd1/lsd_slam/liveframes",1, frameCb);
	ros::Subscriber keyFrames_sub = nh.subscribe("/lsd1/lsd_slam/keyframes",20, frameCb);
	ros::Subscriber graph_sub       = nh.subscribe("/lsd1/lsd_slam/graph",10, graphCb);
	
	// For camera 2
	ros::Subscriber liveFrames_sub2 = nh.subscribe("/lsd2/lsd_slam/liveframes",1, frameCb2);
	ros::Subscriber keyFrames_sub2 = nh.subscribe("/lsd2/lsd_slam/keyframes",20, frameCb2);
	ros::Subscriber graph_sub2       = nh.subscribe("/lsd2/lsd_slam/graph",10, graphCb2);
