...
void PointCloudViewer::addFrameMsg2(lsd_slam_viewer::keyframeMsgConstPtr msg)
{
	meddleMutex.lock();
...
void PointCloudViewer::addGraphMsg2(lsd_slam_viewer::keyframeGraphMsgConstPtr msg)
{
	meddleMutex.lock();
...
void PointCloudViewer::draw()
{
	...
	if(showCurrentCamera){
		...
		currentCamDisplay2->drawCam(2*lineTesselation, 0);
	}    
